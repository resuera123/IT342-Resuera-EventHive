...
function getNotifPrefs(): Record<string, boolean> {
  try {
    const stored = localStorage.getItem('eventhive_notif_prefs')
    if (stored) return JSON.parse(stored)
  } catch {}
  // Defaults: all enabled
  return { registration: true, newParticipant: true, cancellation: true, eventUpdates: true }
}

function isNotifEnabled(type: string): boolean {
...
export default function Navbar({ user: propUser }: NavbarProps) {
  ...
  const handleMarkAllRead = () => {
    fetch('http://localhost:8081/api/notifications/read-all', { method: 'PATCH', credentials: 'include' })
      .then(() => {
  ...
  const handleClickNotif = (notif: NotifItem) => {
    if (!notif.read) {
      fetch(`http://localhost:8081/api/notifications/${notif.id}/read`, { method: 'PATCH', credentials: 'include' })
        .then(() => {
